...
| **多平台 Release** | 每天 1 次打 **3 个汉化包**：Linux x64、Windows x64（.exe）、macOS ARM64 |
...
仅改本仓库翻译、官方版本未变时，需等官方出新版才会触发自动发布；或本地构建后自行处理发布。
...
到 [npm 包页面](https://www.npmjs.com/package/@agentai2027/openclaw-zh) 看有没有版本；或等 CI 跑完「定时检查最新版」且日志出现 `[npm] 发布成功`。
