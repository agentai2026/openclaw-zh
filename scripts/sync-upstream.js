...
/** 仓库内旧备份目录（需清理，避免 cp 路径冲突） */
const LEGACY_BACKUP_DIR = '.sync-overlay-backup';

/**
 * 受保护路径：只列汉化版自有文件，不要整目录覆盖 scripts/（会与官方上千文件冲突）
 */
const PROTECTED_PATHS = [
  '.github/workflows/release.yml',
  '.github/workflows/cleanup-runs.yml',
  'scripts/sync-upstream.js',
  'scripts/apply-i18n.js',
  'scripts/apply-package-zh.js',
  'scripts/bump-version.js',
  'scripts/delete-failed-runs.js',
  'scripts/prepare-npm-publish.js',
  'i18n/',
  'patches/',
  'docker/',
  'deploy/',
...
async function cleanGitState() {
  ...
  await rm(LEGACY_BACKUP_DIR, { recursive: true, force: true });
  await rm(join('scripts', LEGACY_BACKUP_DIR), { recursive: true, force: true }).catch(() => {});
}

async function backupProtectedFiles() {
  backupDir = join(tmpdir(), `openclaw-zh-sync-${Date.now()}`);
  log('backup', `备份汉化文件到 ${backupDir}`);
